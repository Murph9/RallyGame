...
public abstract class DefaultBuilder extends World {
    ...
    public void initialize(Application app) {
        ...
        var mainModel = LoadModelWrapper.create(app.getAssetManager(), type[0].getFileName());
        for (int i = 0; i < type.length; i++) {
            WPObject wpo = new WPObject();
            wpo.wp = type[i];
            wpo.sp = mainModel.getChild(type[i].getName()).clone();

            //scale and unscale spatials so that the collision shape size is correct with custom scaling
