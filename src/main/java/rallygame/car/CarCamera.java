...
public class CarCamera extends BaseAppState implements RawInputListener {
	...
	private Vector2f getDir() {
		...
		return H.v3tov2fXZ(lastPos).normalize();
	}

	@Override
	public void render(RenderManager rm) {
		//get direction of camera in 2d
		Vector2f vec_2 = getDir().normalize();
		...
		nextPos.x = vec_2.x * data.cam_offsetLength;
		nextPos.y = data.cam_offsetHeight; // ignore y last
		nextPos.z = vec_2.y * data.cam_offsetLength;

		nextPos = carPos.add(nextPos);
		...
		lastShake.addLocal(
				new Vector3f(FastMath.nextRandomFloat(), FastMath.nextRandomFloat(), FastMath.nextRandomFloat())
						.normalize().mult(shakeFactor * FastMath.nextRandomInt(-1, 1)));
